...
export const ADMIN_USER_SKINS_QUERY_KEY = [
  'admin',
  'library',
  'user-skins',
] as const
export const ADMIN_USER_CAPES_QUERY_KEY = [
  'admin',
  'library',
  'user-capes',
] as const
...
export async function getAdminUserSkins(
  ...
  return authApiClient.get<AdminSkinListResponse>(
    qs
      ? `/library/admin/users/${userId}/skins?${qs}`
      : `/library/admin/users/${userId}/skins`,
  )
}
...
export async function revokeSkinFromUser(
  ...
  return authApiClient.delete(
    `/library/admin/users/${userId}/skins/${skinLibraryId}`,
  )
}
...
export async function getAdminUserCapes(
  ...
  return authApiClient.get<AdminCapeListResponse>(
    qs
      ? `/library/admin/users/${userId}/capes?${qs}`
      : `/library/admin/users/${userId}/capes`,
  )
}
...
export async function revokeCapeFromUser(
  ...
  return authApiClient.delete(
    `/library/admin/users/${userId}/capes/${capeLibraryId}`,
  )
}

/** 管理员同步用户配额 */
export async function syncUserQuota(userId: string): Promise<void> {
...
export function useRevokeSkinMutation(userId: string) {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (skinLibraryId: string) =>
      revokeSkinFromUser(userId, skinLibraryId),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: [...ADMIN_USER_SKINS_QUERY_KEY, userId],
      })
...
export function useRevokeCapeMutation(userId: string) {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (capeLibraryId: string) =>
      revokeCapeFromUser(userId, capeLibraryId),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: [...ADMIN_USER_CAPES_QUERY_KEY, userId],
      })
