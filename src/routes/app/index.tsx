  ...
  return (
    <div className="flex flex-col gap-6">
      <section className="rounded-[1.25rem] border border-border/70 p-4.5 sm:p-6 bg-gradient-to-b from-card to-card/95 shadow-[0_16px_40px_-28px_oklch(0.18_0.025_195_/_0.18)] relative overflow-hidden">
        ...
        <div className="pointer-events-none absolute bottom-0 left-0 h-28 w-28 rounded-full bg-[var(--hero-b)] blur-3xl" />

        <div className="relative grid gap-4 lg:grid-cols-[minmax(0,1.35fr)_minmax(260px,0.9fr)] lg:items-start">
          <div className="flex flex-col gap-4">
            <div className="inline-flex items-center gap-2 rounded-full border border-[var(--diamond)]/15 bg-background/75 px-3 py-1.5 text-xs font-semibold text-[var(--diamond-deep)] shadow-sm">
            ...
            <div className="flex flex-wrap gap-2.5">
              {spotlightStats.map(({ label, value, icon: Icon }) => (
                <div key={label} className="inline-flex items-center gap-2 rounded-full border border-border/70 bg-background/80 px-3 py-2 shadow-[inset_0_1px_0_oklch(1_0_0_/_0.45)]">
                  <Icon className="size-4 text-[var(--diamond)]" />
          ...
          {quickLinks.map((link) => (
            ...
            >
              <div className="flex items-start justify-between gap-4">
                ...
                <ArrowRight className="size-4 text-muted-foreground/50 transition-transform group-hover:translate-x-1 group-hover:text-[var(--diamond)]" />
              </div>

              <div className="flex flex-1 flex-col gap-2">
                <h3 className="text-base font-semibold text-foreground transition-colors group-hover:text-[var(--diamond)]">
                  {link.title}
                </h3>
